...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="deck">
      ...
      <section className="detail-panel panel"><div className="panel-head"><span>02</span><h2>Asset Ledger</h2></div>{selected ? <AssetLedger asset={selected} projects={projects} onAct={act} /> : <Empty text="选择或创建资产后，项目、端点和记忆关系会在这里展开。" />}</section>
      <aside className="memory-panel panel"><div className="panel-head"><span>03</span><h2>Memory Routing</h2></div>{selected ? <MemoryRouting asset={selected} allMemories={memories} onAct={act} /> : <Empty text="资产建立后，可以将单条记忆显式路由给它。" />}</aside>
    </section>
  </main>;
}
...
function AssetLedger({ asset, projects, onAct }: { asset: Asset; projects: Project[]; onAct: (action: () => Promise<unknown>, success: string) => void }) {
...
function MemoryRouting({ asset, allMemories, onAct }: { asset: Asset; allMemories: Memory[]; onAct: (action: () => Promise<unknown>, success: string) => void }) {
  ...
  return <><p className="routing-copy">显示的是该资产此刻可读的规范记忆。点击“授予”只建立引用，不复制或改变原始记忆。</p><div className="mode-toggle"><button className={mode === "manual" ? "on" : ""} onClick={() => setMode("manual")}>手动</button><button className={mode === "automatic" ? "on" : ""} onClick={() => setMode("automatic")}>自动</button></div><div className="memory-list">{allMemories.map((memory) => <article className={`memory-card ${visible.has(memory.id) ? "visible" : ""}`} key={memory.id}><header><span>{memory.kind}</span><time>{memory.scope.level}{memory.scope.project_id ? `:${memory.scope.project_id}` : ""}</time></header><p>{memory.content}</p><footer><small>{visible.has(memory.id) ? (directGrant.has(memory.id) ? "显式授权" : "由 scope 可见") : "未路由"}</small>{!visible.has(memory.id) && <button onClick={() => onAct(() => api.addGrant(asset.id, memory.id, mode), "记忆已授予该资产")}>授予</button>}</footer></article>)}{allMemories.length === 0 && <Empty text="还没有 active 记忆；可在旧管理页提出并批准第一条。" />}</div></>;
}
